...
import { MyLearningSpacesResponse, Lesson, LessonProgress, QuizSession, QuizResult, LessonNote } from '@/types/space.types';

export const getMyLearningSpaces = async (filter?: 'not_started' | 'in_progress' | 'completed'): Promise<MyLearningSpacesResponse> => {
    try {
        const params = filter ? { filter } : {};
        const response = await api.get('/spaces/owned', { params });
        return response.data as MyLearningSpacesResponse;
    } catch (error: any) {
...
export const getLessons = async (spaceId: string): Promise<Lesson[]> => {
    try {
        const response = await api.get(`/spaces/${spaceId}/lessons`);
