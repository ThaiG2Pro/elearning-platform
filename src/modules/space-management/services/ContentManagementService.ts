...
import { PublicSpaceDto, ChapterDto as PublicChapterDto, LessonDto as PublicLessonDto } from '../dtos/SpaceDetailDto';
...
export class ContentManagementService {
    ...
    async getOwnedSpaces(ownerId: bigint, status?: string | null): Promise<SpaceSummaryDto[]> {
        ...
        return spaces.map(space => {
            ...
            const firstVideoUrl = VideoThumbnailUtil.findFirstVideoUrl(
                space.chapters.map(ch => ({ lessons: ch.lessons.filter(l => l.content_url) })),
            );
    ...
    async createSpace(ownerId: bigint, dto: CreateSpaceDto): Promise<bigint> {
        ...
        return await this.prisma.$transaction(async (tx) => {
            const space = await tx.spaces.create({
                data: {
                    owner_id: ownerId,
                    title,
                    slug,
        ...
        return await this.prisma.$transaction(async (tx) => {
            const space = await tx.spaces.create({
                data: {
                    ...
                    slug,
                    status: 'ACTIVE',
                    // WP1.10.1 — "space sinh từ nguồn nào", một ngữ nghĩa duy
            ...
            const chapter = await tx.chapters.create({
                data: { space_id: space.id, title: 'Chương 1', order_index: 1 },
            });
    ...
    async getPublicSpaceByToken(token: string): Promise<PublicSpaceDto> {
        const space = await this.spaceRepository.findByShareToken(token);
        if (!space) throw new Error('SHARE_LINK_NOT_FOUND');

        const chapters = space.chapters.map((chapter: any) => {
    ...
    /** Clones a shared space into `userId`'s own account ("Sao chép về học"). */
    async cloneSharedSpace(token: string, userId: bigint): Promise<bigint> {
        const space = await this.spaceRepository.findByShareToken(token);
        if (!space) throw new Error('SHARE_LINK_NOT_FOUND');

        return await this.spaceRepository.cloneForOwner(space.id, userId);
    }
    ...
    async updateSpaceMetadata(ownerId: bigint, spaceId: bigint, data: { title?: string; description?: string; status?: 'ACTIVE' | 'ARCHIVED' }): Promise<void> {
        ...
        // WP1.6 follow-up (round 3) — Space.archive()/unarchive() existed in
        // the domain since the ownership pivot but nothing ever called them:
        // no route, no service method, no UI action. The /my-spaces
     ...
     * checked the parent space, most didn't; the service never did).
     ...
     * two helpers resolve the owning space so every mutation below can be
    ...
    private async getOwnerIdForSection(sectionId: bigint): Promise<bigint> {
        const section = await this.prisma.chapters.findUnique({
            where: { id: sectionId },
            select: { space: { select: { owner_id: true } } },
        });
        if (!section) throw new Error('SECTION_NOT_FOUND');
        return section.space.owner_id;
    }

    private async getOwnerIdForLesson(lessonId: bigint): Promise<bigint> {
        const lesson = await this.prisma.lessons.findUnique({
            where: { id: lessonId },
            select: { chapter: { select: { space: { select: { owner_id: true } } } } },
        });
