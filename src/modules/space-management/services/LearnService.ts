...
export class LearnService {
    ...
    async trackVideoProgress(
        ...
        // owner-gated /spaces/[id]/lessons list), but this endpoint takes a
    ...
    async getProgress(userId: bigint, lessonId: bigint): Promise<{ currentPosition: number; isCompleted: boolean; lastAccessedAt: string } | null> {
        ...
        // given lessonId exists/belongs to a space at all via this route.
        const spaceId = await this.findSpaceIdByLesson(lessonId);
        await this.assertOwnership(spaceId, userId);
    ...
    /** WP1.3 — % of a space's lessons the given user has finished, for space-detail display. */
    async getSpaceProgress(userId: bigint, spaceId: bigint): Promise<{ completionRate: number; finishedLessons: number; totalLessons: number }> {
        const lessons = await this.prisma.lessons.findMany({
            where: { chapter: { space_id: spaceId } },
            select: { id: true },
        });
        const totalLessons = lessons.length;

        const progresses = await this.progressRepo.findByUserAndSpace(userId, spaceId);
    ...
    /** Personal-organizer model: only the space's owner may read/write progress on its lessons. */
    private async assertOwnership(spaceId: bigint, userId: bigint): Promise<void> {
