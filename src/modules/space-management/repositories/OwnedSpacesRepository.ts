...
// WP1.6.3 — findByStudentAndSpace/findByStudent/findById/save (reads/writes
...
// Only getOwnedSpacesWithDetails remains — it now reads owned spaces
...
// reads spaces the user owns and folds in their learning progress for the
// /my-learning and homepage "continue learning" views.
export class OwnedSpacesRepository {
    ...
    // since the ownership pivot (WP0.2): space access/learn/lessons are all
    // gated on owner_id now (see AccessControlPolicy), so `/my-learning` was
    // permanently empty even for users with real, owned spaces. This now
    // mirrors ContentManagementService.getOwnedSpaces' owner_id query
    // and computes progress from `learning_progress` the same way
    // LearnService.getSpaceProgress does for space-detail — one definition
    // of "done" across the app instead of two disconnected ones.
    async getOwnedSpacesWithDetails(userId: bigint, filter?: string | null, sort?: string | null): Promise<OwnedSpaceDto[]> {
            ...
            include: {
                chapters: {
                    ...
                    include: {
                        ...
                        // finishedLessons, so a space could show 100%/
                        // "completed" here purely from watched videos while
                        // its quiz was never attempted — inconsistent with
                        // LearnService.getSpaceProgress (used on the
                        // space-detail page), which has no such filter and
        ...
        const lessonIdsBySpace = new Map<bigint, Set<bigint>>();
        ...
        const results: OwnedSpaceDto[] = spaces.map(space => {
            ...
            const lessonIds = lessonIdsBySpace.get(space.id)!;
