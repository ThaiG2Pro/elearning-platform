...
// `questions.options` là jsonb array of strings (schema hardening 2026-08-21,
// thay 4 cột option_a..d cứng) — không còn pad-'' hay trần 4 đáp án; chỉ cần
// guard kiểu vì jsonb về mặt lý thuyết chứa được JSON bất kỳ.
function buildOptions(options: unknown): string[] {
...
export class QuestionRepository {
    ...
    async findRandomByLesson(lessonId: bigint, limit: number = 10): Promise<Question[]> {
        ...
        const domainQuestions = questions.map(q =>
            new Question(q.id, q.lesson_id, q.content, buildOptions(q.options), q.answer_key)
        );
    ...
    async replaceAllForLesson(lessonId: bigint, newQuestions: Omit<Question, 'id'>[]): Promise<void> {
        ...
        // two independent awaits, so a failure in createMany (DB blip, etc.)
