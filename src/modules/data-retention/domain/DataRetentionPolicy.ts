...
export interface ArchiveEligibilityInput {
    ...
    createdAt: Date | null;
    /**
     * true nếu còn ít nhất 1 course công khai (showcase HOẶC có share_token)
...
export class DataRetentionPolicy {
    /**
     * Không bao giờ archive nếu còn course công khai nào tham chiếu — dù đã
    ...
    static isEligibleForArchive(input: ArchiveEligibilityInput): boolean {
        if (input.hasPublicCourseReference) {
            return false;
        }
